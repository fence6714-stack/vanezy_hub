--[[
	SYNAPSE HUB v29 - COMPLETE ULTIMATE EDITION
	by Vanezy Scripts
]]

print("START - Synapse Hub v29")
...
local speedHackSlider, espSizeSlider
...
local function loadValue(n, d) local v = StorageValues:GetAttribute(n); return v ~= nil and v or d end
local function saveValue(n, v) StorageValues:SetAttribute(n, v) end
...
for i = 1, 6 do
	...
	if d then
		local posData = {d.posX, d.posY, d.posZ, d.posX, d.posY, d.posZ, d.posX, d.posY, d.posZ, d.posX, d.posY, d.posZ}
		teleportPoints[i] = {name = d.name or ("home "..i), pos = CFrame.new(unpack(posData))}
	end
end
local function saveTP()
	for i = 1, 6 do
		if teleportPoints[i] then
			...
			saveValue("TP_" .. i, {name = teleportPoints[i].name, posX = p.X, posY = p.Y, posZ = p.Z})
		else
			saveValue("TP_" .. i, nil)
		end
	end
...
CombatContainer.CanvasSize = UDim2.new(0,0,0,320)
...
CombatContent.Size = UDim2.new(1,0,0,340)
...
ESPContent.Size = UDim2.new(1,0,0,380)
...
TeleportContainer.CanvasSize = UDim2.new(0,0,0,500)
...
TeleportContent.Size = UDim2.new(1,0,0,520)
...
ResetBtn.MouseButton1Click:Connect(function()
	...
	settings.windowWidth = 450; settings.windowHeight = 420
...
trollY = trollY + 48
trollBtn("🔒 JAIL (10 sec)", Color3.fromRGB(200,100,0), function(t)
	local c = t.Character
	local h = c and c:FindFirstChild("HumanoidRootPart")
	if h then
		local j = Instance.new("Part")
		j.Size = Vector3.new(5,5,5)
		j.Position = h.Position
		j.Anchored = true
		j.CanCollide = true
		j.Transparency = 0.5
		...
		box.Adornee = j
		t.Character.HumanoidRootPart.CFrame = j.CFrame
		task.delay(10, function() j:Destroy() end)
		createNotification("🔒 Jailed "..t.Name,1.5)
	end
end)
trollY = trollY + 48
trollBtn("💥 EXPLODE", Color3.fromRGB(200,80,0), function(t)
	local c = t.Character
	local h = c and c:FindFirstChild("HumanoidRootPart")
	if h then
		...
		e.BlastRadius = 5
		e.Parent = Workspace
		createNotification("💥 Boomed "..t.Name,1.5)
	end
end)
trollY = trollY + 48
trollBtn("💨 SLAP", Color3.fromRGB(180,120,0), function(t)
	local c = t.Character
	local h = c and c:FindFirstChild("HumanoidRootPart")
	if h then h.Velocity = Vector3.new(0,50,0); createNotification("💨 Slapped "..t.Name,1.5) end
end)
trollY = trollY + 48
trollBtn("🔄 LOOP KICK", Color3.fromRGB(200,60,60), function(t)
	for i=1,5 do task.wait(0.2) pcall(function() t:Kick("🔁 Loop kicked") end) end
	createNotification("🔄 Loop kicking "..t.Name,1.5)
end)
trollY = trollY + 48
trollBtn("👁️ BLIND", Color3.fromRGB(100,100,200), function(t)
	local head = t.Character and t.Character:FindFirstChild("Head")
	if head then
		local b = Instance.new("Part")
		b.Size = Vector3.new(10,10,10)
		b.Position = head.Position + Vector3.new(0,0,5)
		b.Anchored = true
		b.CanCollide = false
		b.Transparency = 0.3
		...
		createNotification("👁️ Blinded "..t.Name,1.5)
	end
end)
trollY = trollY + 48
trollBtn("🔨 FAKE BAN", Color3.fromRGB(200,50,50), function(t)
	pcall(function() t:Kick("🔨 You have been banned (jk)") end)
	createNotification("🔨 Fake banned "..t.Name,1.5)
end)
trollY = trollY + 56
createSection(TrollContent, "——— ALL ———", trollY); trollY = trollY + 36
trollBtn("💀 KILL ALL (DANGER)", Color3.fromRGB(180,0,0), function()
	for _,p in pairs(Players:GetPlayers()) do if p~=player then local c=p.Character; if c and c:FindFirstChild("Humanoid") then c.Humanoid.Health=0 end end end
	createNotification("💀 Killed all",1.5)
end)
trollY = trollY + 48
local loopKillAllBtn = trollBtn("🔄 KILL LOOP (DANGER)", Color3.fromRGB(150,0,0), function()
	...
	if killLoopActive then
		...
		createNotification("🔄 Kill loop ON",1.5)
	else
		if killLoopConnection then killLoopConnection:Disconnect() end
		createNotification("🔄 Kill loop OFF",1.5)
	end
	loopKillAllBtn.Text = killLoopActive and "🔄 KILL LOOP: ON" or "🔄 KILL LOOP: OFF"
end)
loopKillAllBtn.Text = "🔄 KILL LOOP: OFF"
trollY = trollY + 48
trollBtn("🔒 SERVER LOCK", Color3.fromRGB(0,100,150), function()
	game.Players.PlayerAdded:Connect(function(plr) pcall(function() plr:Kick("🔒 Server locked by admin") end) end)
	createNotification("🔒 Server locked",1.5)
end)
trollY = trollY + 48
trollBtn("💬 CHAT SPAM (10x)", Color3.fromRGB(150,100,50), function()
	for i=1,10 do
		pcall(function() ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("Vanezy Scripts is here! https://t.me/VanezyScripts","All") end)
		task.wait(0.1)
	end
	createNotification("💬 Spam sent",1.5)
end)
trollY = trollY + 60
...
createNotification("✨ Synapse Hub v29 Ready! ✨", 2.5)
print("✅ Synapse Hub v29 loaded - COMPLETE ULTIMATE VERSION")
